...
def get_data(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
             test_start=0, valid_portion=0.3, prefix="", size=None, tr_type=None):
    print("prefix", prefix)
    """
    ...
    x_dim = get_data_dim(dataset, size)
    
    #f = open(os.path.join(prefix, dataset + '_train.pkl'), "rb")
    f = open(os.path.join(prefix, dataset + '_' + tr_type + '_train_' + str(size)+ '.pkl'), "rb")#   'random_train_10.pkl'), "rb")
    #f = open(os.path.join(prefix, dataset + '_random_train_10.pkl'), "rb")
    #f = open(os.path.join(prefix, dataset + '_least_score_train_10.pkl'), "rb")
    #f = open(os.path.join(prefix, dataset + '_feature_importance_train_10.pkl'), "rb")
    ...
    try:
        ...
        f = open(os.path.join(prefix, dataset + '_' + tr_type + '_test_' + str(size) + '.pkl'), "rb")#   '_random_test_10.pkl'), "rb")
        #f = open(os.path.join(prefix, dataset + '_random_test_10.pkl'), "rb")
        #f = open(os.path.join(prefix, dataset + '_least_score_test_10.pkl'), "rb")
        #f = open(os.path.join(prefix, dataset + '_feature_importance_test_10.pkl'), "rb")
        test_data = pickle.load(f)
        test_data = np.array(test_data)
        test_data = test_data.reshape((-1, x_dim))[test_start:test_end, :]
        f.close()
    except (KeyError, FileNotFoundError):
        test_data = None
    try:
        #f = open(os.path.join(prefix, dataset + "_label.pkl"), "rb")
        f = open(os.path.join(prefix, dataset + "_test_label.pkl"), "rb")
        #f = open(os.path.join(prefix, dataset + "_A1_test_label.pkl"), "rb")
