...
df = pd.read_csv('./data/SWaT/SWaT_Dataset_Attack_v0.csv')
...
df = pd.read_csv('./data/SWaT/SWaT_Dataset_Normal_v0.csv')
...
src_path = '../data/WADI/'
a = str(open(f'{src_path}WADI_14days.csv', 'rb').read(), encoding='utf8').split('\n')[5: -1]
a = '\n'.join(a)
...
pkl.dump(train, open('WADI_A1_train.pkl', 'wb'))
print('WADI_train saved')

df = pd.read_csv(f'{src_path}WADI_attackdata_wlabel.csv')
label = df['label']
df.drop(['label'], axis=1, inplace=True)
test = df.to_numpy()[:, 3:]
test = np.delete(test, nan_cols, axis=1)
assert test.shape == (172801, 118)
pkl.dump(test, open('WADI_A1_test.pkl', 'wb'))
print('WADI_test saved')

pkl.dump(label, open('WADI_A1_test_label.pkl', 'wb'))
