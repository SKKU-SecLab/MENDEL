...
def get_data(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
             test_start=0, valid_portion=0.3, prefix="./data/processed/", size=None, tr_type=None):
    """
    ...
    f = open(os.path.join(prefix, 'SWaT-WADI_' + dataset + '_' + tr_type + '_train_' + str(size) + '.pkl'), "rb")
    train_data = pickle.load(f)
    train_data = np.array(train_data)
    train_data = train_data.reshape((-1, x_dim))[train_start:train_end, :]
    f.close()
    try:
        f = open(os.path.join(prefix, 'SWaT-WADI_' + dataset + '_' + tr_type + '_test_' + str(size) + '.pkl'), "rb")
