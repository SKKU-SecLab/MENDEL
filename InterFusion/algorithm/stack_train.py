...
class ExpConfig(mltk.Config):
    ...
    def _model_post_checker(self, v: 'ExpConfig'):
        if v.model.x_dim == -1:
            v.model.x_dim = get_data_dim(v.dataset, v.transfer_size)
        if v.dataset == 'SWaT' or v.dataset == 'HAI':
            v.model.z_dim = 2
        if v.dataset == 'WADI':
            v.model.z_dim = 4
    ...
    def _train_post_checker(self, v: 'ExpConfig'):
        if v.dataset == 'SWaT' or v.dataset == 'WADI' or v.dataset == 'HAI':
...
def main(exp: mltk.Experiment[ExpConfig], config: ExpConfig):
    ...
    (x_train, _), (x_test, y_test) = \
        get_data(config.dataset, config.train.max_train_size, config.test.max_test_size,
                 ...
                 valid_portion=config.train.valid_portion, prefix="./data/processed", size=config.transfer_size, tr_type=config.transfer_type)
    ...
    if config.dataset == 'SWaT' or config.dataset == 'WADI' or config.dataset == 'HAI':
    ...
    var_initializer = tf.variables_initializer(tf.global_variables()) # stack_train version
    #var_initializer = tf.global_variables_initializer() # stack_train2 version
    ...
    with spt.utils.create_session().as_default() as session:
        ...
        if config.transfer:
            ...
            if os.path.exists(os.path.join(exp.config.exp_dir_load_path, 'result_params/restored_pretrain_params.dat.meta')):
                saver.restore(session, os.path.join(exp.config.exp_dir_load_path, 'result_params/restored_pretrain_params.dat'))
            with pre_loop:
                pre_trainer.run()
